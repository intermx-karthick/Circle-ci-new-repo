...
describe('CreateMarketPlanWithTwoAudience', function () {
    ...
    it('MarketPlanCreationWithTwoAudience', function () {
        ...
        workspacepage.clickOnAudienceInParameters(this.data.persons_18_plus)

        
        workspacepage.clickOnPlanPeriod()
        cy.wait(5000)
        cy.get(".market-list-tip>li>div>div").contains(this.data.Persons_18plus_yrs).should('be.visible')
        cy.get("div:nth-child(5) div:nth-child(1) div:nth-child(2)").contains(this.data.two_week).should('be.visible')
        cy.wait(3000)
        workspacepage.clickOnReGenerateButton()
        cy.wait(8000)
        cy.get('mat-cell').contains('(202106)Persons 5+ yrs').should('have.text', (this.data.Persons_18plus_yrs))
        workspacepage.deleteCreatedProject1(campName)

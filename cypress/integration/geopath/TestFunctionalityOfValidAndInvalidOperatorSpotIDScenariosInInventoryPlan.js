...
describe('TestFunctionalityOfValidAndInvalidGeopathIDScenarios', function () {  
    ...
    it('1. Test the functionality Submit Valid and Invalid Operator Ids on Inventory Plan popup and Validate', function () {
        cy.wait(5000)
        ...
        cy.log("Step 5: Submit the valid 5 Operator Spot Id and Validate in the Summary page")
        cy.log("Step 6: Again Reopen the Inventory Plan popup and Submit the 5 Invalid and 5 valid Operator Ids and Validate the Invalid ID validation")
        cy.log("Step 7: Remove the Invalid IDs and Apply with the Valid Operator IDs")
        cy.log("Step 8: Then Validate total of operartor ids in the Summary page")
        ...
        workspacepage.Submit5ValidOperatorIDandValidateInSummary(this.data.Valid_OperatorID)
        workspacepage.Submit5Invalidand5validOperatorIDandValidateInSummary(this.data.InValid_OperatorID)
        workspacepage.RemoveInvalidOperatorIdAndApplywithValidIdandValidateInSummary()
        workspacepage.GenerateInventoryPlanAndValidateUnitScenarioNameinNotificationandredirectsToInventoryPlanusingOperatorID()
    })

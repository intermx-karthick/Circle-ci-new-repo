import { ChangeDetectorRef, OnDestroy, ViewChild , Component, Inject, EventEmitter, Output} from '@angular/core';
import {
  Note,
  NotePagination,
  NoteMessage
...
import { debounceTime, map, tap, filter, takeUntil, catchError } from 'rxjs/operators';
...
export abstract class  NoteAbstract implements OnDestroy{
  ...
  loadMoreNoteData(parentNoteId){
    ...
    this.cdRef.markForCheck();
    this.recordService
      .getNoteDetailsId(parentNoteId, this.notePagination, this.moduleName,this.organizationId, true)
      .pipe(
  ...
  loadNotes(parentId){
    ...
    this.cdRef.markForCheck();
     this.recordService
      .getNoteDetailsId(parentId, this.notePagination, this.moduleName, this.organizationId,true)
      .pipe(takeUntil(this.unSubscribe$))
      .subscribe((response) => {
        this.isLoadingNote = false;
        if (response) {
          this.noteList = response?.['results'] ?? [];
          if(this.noteList.length<1 && this.dialogRef?._containerInstance?._config?.panelClass === 'note-dialog-main'){
  ...
  public updateNote(note){
    ...
    if (noteValue && typeof noteValue === 'string' && noteValue.trim().length < 1) {
        this.formNote[note._id].setErrors({invalid: true});
        this.cdRef.markForCheck();
        return;
    } else if(noteValue && typeof noteValue === 'string' && noteValue.trim().length > 2000) {
  ...
  public  createNote() {
    ...
    this.submitAPI = true;
    this.recordService
      .createNoteByModuleName(this.parentNoteId, formValue, this.moduleName, this.organizationId)
      .pipe(takeUntil(this.unSubscribe$))
      .subscribe((response) => {
        this.submitAPI = false;       
        if (response) {
         this.showsAlertMessage(`${NoteMessage[this.moduleName]} note created successfully!`);
  ...
  onDeleteNote(note:Note){
     this.recordService
        .deleteNoteByModuleName(this.parentNoteId, note._id, this.moduleName, this.organizationId)
        .pipe(takeUntil(this.unSubscribe$))
        .subscribe((response) => {
          this.submitAPI = false;       
          if (response) {
          this.showsAlertMessage(`${NoteMessage[this.moduleName]} note deleted successfully!`);
          this.noteUpdateEmit.emit({'noteUpdated':true});
          this.resetNotePagination();
          this.loadNotes(this.parentNoteId);
          }
  ...
  public  updateNoteById(note:Note, noteValue){
    const payload = {
      ...
      this.recordService
        .updateNoteByModuleName(this.parentNoteId, note._id, payload, this.moduleName, this.organizationId)
        .pipe(takeUntil(this.unSubscribe$))
        .subscribe((response) => {
          this.submitAPI = false; 
          if (response) {
          this.showsAlertMessage(`${NoteMessage[this.moduleName]} note updated successfully!`);
          this.resetNotePagination();
          this.loadNotes(this.parentNoteId);
          this.noteUpdateEmit.emit({'noteUpdated':true});
  ...
  resetNotePagination() {
    if(this.notePagination?.total){
      this.notePagination = { page: 1, perPage: 10 , total:this.notePagination?.total };
    }else{

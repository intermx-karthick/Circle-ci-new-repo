...
const radiusPopup = MapboxFactory.produce(MapboxFactoryEnum.POPUP, {closeButton: false});
...
function createGeoJSONCircle(center, radiusInKm, parentId, points = 64) {
  ...
  ret.push(ret[0]);
  RadiusMode.circleCoordinates = ret;
  return {
    type: 'Feature',
    geometry: {
      type: 'Polygon',
...
function getDisplayMeasurements(feature) {
  ...
  RadiusMode.radius = displayMeasurements.standard;
...
RadiusMode.onStop = function(state) {
  ...
  if (state.line.isValid()) {
    ...
    const pointWithRadius = createGeoJSONCircle(lineGeoJson.geometry.coordinates[0], (lineDistance(lineGeoJson)).toFixed(1), null, 64);
...
RadiusMode.toDisplayFeatures = function(state, geojson, display) {
  ...
  if (RadiusMode.circleCoordinates.length > 0 ) {
    for (let i = 0; i < RadiusMode.circleCoordinates.length ; i++ ) {
        radiusPopup
            .setLngLat(RadiusMode.circleCoordinates[i])
            .setHTML('<div class="circle-radius">' + RadiusMode.radius + '</div>')
            .addTo(this.map);
    }
  }

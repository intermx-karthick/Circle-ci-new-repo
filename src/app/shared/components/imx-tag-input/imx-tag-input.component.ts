...
@Component({
  selector: 'app-imx-tag-input',
  templateUrl: './imx-tag-input.component.html',
  styleUrls: ['./imx-tag-input.component.less'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ImxTagInputComponent implements OnInit, OnChanges {
  ...
  @Output() clearAllUnitIds = new EventEmitter();
  @Input() validChips: any;
  ...
  checkingValidChips(chip: any) {
    if(this.validChips) {
      if(this.validChips.includes(chip)) {
        return true;
      }
    }
    return false;
  ...
  clearAll() {
    this.clearAllUnitIds.emit(name);
  }
}
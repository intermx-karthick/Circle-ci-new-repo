...
export abstract class AbstractMediaFilterW3<
  T extends any
> extends AbstractLazyLoadComponent {
  @Input() moduleName: string;
  @Input() mediaTypesDataForEdit: any;
  @Input() clearData: Subject<void> = new Subject<void>();
  @Output() select = new EventEmitter();

  dataSource: T[] = [];
  selected: T[] = [];
  isInitialLoadCompleted: boolean = false;
  unsubscribeInitiator$: Subject<any> = new Subject<any>();
  ...
  protected loadDataSource(filter = {}, selected = []) {
    ...
    let customInventoy: Observable<any> = this.queryFromCustomInventoryESService(
      filters
    );
    forkJoin([
  ...
  protected clearProjectInventory() {
    if (this.moduleName === 'project') {
      setTimeout(() => {

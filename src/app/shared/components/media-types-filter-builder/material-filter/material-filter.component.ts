...
export class MaterialFilterComponent extends AbstractMediaFilter<any> implements OnInit, OnDestroy {
  ...
  static formatElasticResults(aggregated) {
    ...
    if (aggregated['digitals'] && aggregated['digitals']['buckets']) {
      ...
      result['digital'] = digital && digital['spots']['spot_filter']['spot_count']['value'] || 0;
      result['non-digital'] = nonDigital && nonDigital['spots']['spot_filter']['spot_count']['value'] || 0;
    }
    return result;
  }
  ...
  formatCustomInventoryData(response): any {
    let data = {};
    data['digital'] = MaterialFilterComponent.formatElasticResults(response);
    data['medias'] = MaterialFilterComponent.formatMediaTypeResults(response);
    return data;
  }
  ...
  handleAPIsResponse([summaries, elastic]: readonly [any, any], selected): void {
    ...
    summaries.map(summary => {
      if (summary['summarizes']
        && summary['summarizes']['summarizes_id_list']
        && summary['summarizes']['summarizes_id_list'][1]) {
        if ((summary['summarizes']['summarizes_id_list'][0]['name']).toLowerCase() === 'true') {
          digitalCount += summary['spots'];
          materialMedias['digital'].push(summary['summarizes']['summarizes_id_list'][1]['name']);
        } else {
          materialMedias['nondigital'].push(summary['summarizes']['summarizes_id_list'][1]['name']);
    ...
    if (this.mediaTypesDataForEdit.editData) {
      ...
      if (
        ...
        if (this.mediaTypesDataForEdit.editData['isDigital'] && this.mediaTypesDataForEdit.editData['isNonDigital']) {
          ...
          this.selectedMaterial = this.mediaTypesDataForEdit.editData['isDigital'] && 'true' || 'false';
        }
      }
    }
